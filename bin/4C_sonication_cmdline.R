...
file_sai1<-paste(file_in1,".sai",sep="")
file_sai2<-paste(file_in2,".sai",sep="")

file_sam1<-paste(file_in1,".sam",sep="")
file_sam2<-paste(file_in2,".sam",sep="")

file_bam1<-paste(file_in1,".bam",sep="")
file_bam2<-paste(file_in2,".bam",sep="")

file_sort1<-paste(file_in1,".sort",sep="")
file_sort2<-paste(file_in2,".sort",sep="")

file_sort_bam1<-paste(file_sort1,".bam",sep="")
file_sort_bam2<-paste(file_sort2,".bam",sep="")
...
file2_merge_filter_bed<-"paired_end_dist_sort_merge_filter2.bed"
file2_merge_score_bed<-"paired_end_dist_sort_merge_filter2_Z.bed"
...
system(paste(path_bwa, "/bwa samse ", genome, " ", file_sai1, " FASTQ_FILTERED1.fq > ", file_sam1, sep=""))
...
system("cp sonic_sort_unique.bam MAPPED_BAM.bam")
...
system(paste(path_bedtools, "/windowBed -a ", file2_merge_bed, " -b bait.bed -u -w 10000 > local.bed", sep=""))
...
system(paste(path_bedtools, "/windowBed -a ", file2_merge_score_bed, " -b positive_hits_intra.bed -u -w ", window_span_intra, " > SIGNIFICANT_SITES_intra.bed",sep=""))
system(paste(path_bedtools, "/windowBed -a ", file2_merge_score_bed, " -b positive_hits_inter.bed -u -w ", window_span_inter, " > SIGNIFICANT_SITES_inter.bed",sep=""))
...
sink()
system("wc fastq_convert.fqp | awk '{print \"Total number of sequenced read pairs = \",$1/4}' >> summary_report.txt")
system("wc FASTQ_FILTERED.fqp | awk '{print \"Total number of sequenced read pairs with good base quality = \",$1/4}' >> summary_report.txt")
...
legend("topright",c("4C sites", "Random sites"), cex=0.7, fill=c("red","blue"))
	...
	if (control == "yes") {
		...
		for (i in 1:11) {
			...
			if (file.exists(chip_file)) {
 				system(paste(path_bedtools, "/windowBed -a SIGNIFICANT_SITES.bed -b ", chipfile[i], ".bed -c > ", chipfile[i], ".txt", sep=""))
				system(paste(path_bedtools, "/windowBed -a ", random_genome, " -b ", chipfile[i], ".bed -c > ", chipfile[i],"_rand.txt", sep=""))
